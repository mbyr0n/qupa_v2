...
namespace argos {
   ...
   static const CRadians FOOTBOT_PROXIMITY_SENSOR_ANGLES[6] = {
      ...
      try {
         ...
         for(size_t i = 0; i < 6; ++i) {
             m_pcProximitySensorEquippedEntity->AddSensor(
                 CVector3(PROXIMITY_SENSOR_RING_RADIUS * Cos(FOOTBOT_PROXIMITY_SENSOR_ANGLES[i]),
                          PROXIMITY_SENSOR_RING_RADIUS * Sin(FOOTBOT_PROXIMITY_SENSOR_ANGLES[i]),
                          PROXIMITY_SENSOR_RING_ELEVATION),
                 // CORRECTO: Pasa un CVector3 como dirección
                 CVector3(Cos(FOOTBOT_PROXIMITY_SENSOR_ANGLES[i]),
                          Sin(FOOTBOT_PROXIMITY_SENSOR_ANGLES[i]),
                          0.0f),
