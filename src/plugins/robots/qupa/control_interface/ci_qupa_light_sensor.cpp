/**
 * @file <argos3/plugins/robots/qupa/control_interface/ci_qupa_light_sensor.cpp>
 *
 * @author Jose Santos
 * Gabriel Madroñero 
 * - <emails >
...
#include "ci_qupa_light_sensor.h"
...
namespace argos {
   ...
   CCI_QupaLightSensor::CCI_QupaLightSensor() :
      m_tReadings(24) {
   ...
   const CCI_QupaLightSensor::TReadings& CCI_QupaLightSensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_QupaLightSensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_QupaLightSensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_QupaLightSensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_QupaLightSensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
