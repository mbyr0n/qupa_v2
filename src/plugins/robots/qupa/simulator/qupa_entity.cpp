...
namespace argos {
   ...
   static UInt32 GetMarkerIdFromEntityId(const std::string& str_id,
                                         bool& b_has_trailing_digits) {
      size_t unMarkerStart = str_id.size();
      while(unMarkerStart > 0 && str_id[unMarkerStart - 1] >= '0' && str_id[unMarkerStart - 1] <= '9') {
         --unMarkerStart;
      }
      b_has_trailing_digits = (unMarkerStart < str_id.size());
      UInt32 unMarkerId = 0;
      for(size_t i = unMarkerStart; i < str_id.size(); ++i) {
         unMarkerId = unMarkerId * 10 + str_id[i] - '0';
      }
      return unMarkerId;
   }

   static UInt32 GetMarkerIdFromEntityId(const std::string& str_id) {
      bool bHasTrailingDigits = false;
      return GetMarkerIdFromEntityId(str_id, bHasTrailingDigits);
   }
   ...
   /* Rango de los rayos de proximidad 50cm */
   ...
   CQupaEntity::CQupaEntity(const std::string& str_id,
      ...
      m_pcWheeledEntity(nullptr),
      m_pcBatteryEquippedEntity(nullptr),
      m_unMarkerId(GetMarkerIdFromEntityId(str_id)) {
      try {
         /*
         ...
         /* RAB equipped entity */
          ...
          /* Native ARGoS tag equipped entity is optional in the programmatic constructor. */
   ...
   void CQupaEntity::Init(TConfigurationNode& t_tree) {
      try {
         ...
         bool bHasTrailingDigits = false;
         m_unMarkerId = GetMarkerIdFromEntityId(strId, bHasTrailingDigits);
         bool bMarkerIdAttribute = false;
         try {
            std::string strMarkerId;
            GetNodeAttribute(t_tree, "marker_id", strMarkerId);
            bMarkerIdAttribute = true;
            GetNodeAttribute(t_tree, "marker_id", m_unMarkerId);
         }
         catch(CARGoSException& ex) {
            if(bMarkerIdAttribute) {
               throw;
            }
         }
         if(!bMarkerIdAttribute && !bHasTrailingDigits) {
            LOGERR << "[WARNING] QUPA entity \"" << strId << "\" has no trailing digits; marker_id defaults to 0." << std::endl;
         }
         /*
          * Create and init components
          */
...
#define UPDATE(COMPONENT) if(COMPONENT->IsEnabled()) COMPONENT->Update();
   ...
   void CQupaEntity::UpdateComponents() {
     ...
     UPDATE(m_pcGripperEquippedEntity);
      ...
      if(m_pcTagEquippedEntity != nullptr) UPDATE(m_pcTagEquippedEntity);
