/**
 * @file <argos3/plugins/robots/qupa/simulator/qupa_distance_scanner_rotzonly_sensor.cpp>
 *
 * @author Jose Santos
 * Gabriel Madroñero    
 */

#include "qupa_distance_scanner_rotzonly_sensor.h"
...
namespace argos {
   ...
   CQupaDistanceScannerRotZOnlySensor::CQupaDistanceScannerRotZOnlySensor() :
      m_pcRNG(nullptr),
      m_bAddNoise(false),
   ...
   void CQupaDistanceScannerRotZOnlySensor::Init(TConfigurationNode& t_tree) {
      try {
         CCI_QupaDistanceScannerSensor::Init(t_tree);
         /* Show rays? */
         ...
         THROW_ARGOSEXCEPTION_NESTED("Initialization error in qupa distance scanner rot_z_only sensor.", ex);
      }
   }
   ...
   void CQupaDistanceScannerRotZOnlySensor::SetRobot(CComposableEntity& c_entity) {
      ...
      m_pcDistScanEntity = &(static_cast<CQupaEntity&>(c_entity).GetDistanceScannerEquippedEntity());
   ...
   void CQupaDistanceScannerRotZOnlySensor::Update() {
      ...
      if(m_pcDistScanEntity->GetMode() != CQupaDistanceScannerEquippedEntity::MODE_OFF) {
         /* Update the readings wrt to device mode */
         if(m_pcDistScanEntity->GetMode() == CQupaDistanceScannerEquippedEntity::MODE_POSITION_CONTROL) {
   ...
   void CQupaDistanceScannerRotZOnlySensor::Reset() {
      /* Clear the maps */
      m_tReadingsMap.clear();
      m_tShortReadingsMap.clear();
      m_tLongReadingsMap.clear();
   ...
   void CQupaDistanceScannerRotZOnlySensor::UpdateNotRotating() {
      /* Short range [0] */
...
#define ADD_READINGS(RAYS,MAP,MINDIST)          \
   ...
   void CQupaDistanceScannerRotZOnlySensor::UpdateRotating() {
   ...
   Real CQupaDistanceScannerRotZOnlySensor::CalculateReadingForRay(const CRay3& c_ray,
                                                                      Real f_min_distance) {
...
#define CALCULATE_LONG_RANGE_RAY(ANGLE,INDEX)               \
   ...
   void CQupaDistanceScannerRotZOnlySensor::CalculateRaysNotRotating() {
      /* We make the assumption that the qupa is rotated only around Z */
      /* Get the qupa orientation */
      ...
      /* The short range sensors are oriented along the qupa local X */
      m_cDirection = CVector3::X;
      CALCULATE_SHORT_RANGE_RAY(cAbsoluteOrientation, 0);
      /* The short range sensors are oriented along the qupa local Y */
   ...
   void CQupaDistanceScannerRotZOnlySensor::CalculateRaysRotating() {
      /* We make the assumption that the qupa is rotated only around Z */
      /* Get the qupa orientation */
      CRadians cTmp1, cTmp2, cOrientationZ;
      m_pcEmbodiedEntity->GetOriginAnchor().Orientation.ToEulerAngles(cOrientationZ, cTmp1, cTmp2);
      ...
      /* The long range sensors are oriented along the qupa local Y */
      m_cDirection = CVector3::Y;
      CALCULATE_LONG_RANGE_RAY(cAbsoluteOrientation, 0);
   ...
   REGISTER_SENSOR(CQupaDistanceScannerRotZOnlySensor,
                   ...
                   "the qupa is always parallel to the XY plane, i.e., it rotates only around\n"
                   "the Z axis. This implementation is faster than a 3D one and should be used\n"
                   "only when the assumption about the qupa rotation holds.\n\n"
                   ...
                   "      <sensors>\n"
                   "        ...\n"
                   "        <qupa_distance_scanner implementation=\"rot_z_only\" />\n"
                   "        ...\n"
                   "      </sensors>\n"
                   "      ...\n"
                   "    </my_controller>\n"
