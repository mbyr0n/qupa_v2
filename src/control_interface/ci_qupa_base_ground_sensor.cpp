...
/**
 * @file <argos3/plugins/robots/qupa/control_interface/ci_qupa_base_ground_sensor.cpp>
 *
 * @brief This file provides the implementation of the qupa base ground sensor.
 *
...
namespace argos {
   ...
   CCI_QupaBaseGroundSensor::CCI_QupaBaseGroundSensor() :
      m_tReadings(8) {
   ...
   const CCI_QupaBaseGroundSensor::TReadings& CCI_QupaBaseGroundSensor::GetReadings() const {
     return m_tReadings;
   }
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_QupaBaseGroundSensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_QupaBaseGroundSensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_QupaBaseGroundSensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_QupaBaseGroundSensor::ReadingsToLuaState(lua_State* pt_lua_state) {
