/**
 * @file <argos3/plugins/robots/qupa/control_interface/ci_qupa_turret_actuator.cpp>
 *
 * @author Carlo Pinciroli - <ilpincy@gmail.com>
 */

#include "ci_qupa_turret_actuator.h"
...
namespace argos {
  ...
  const CRange<SInt32> CCI_QupaTurretActuator::SPEED_RANGE(-4,4);
  const CRange<Real>   CCI_QupaTurretActuator::NORMALIZED_SPEED_RANGE(-1.0,1.0);
  ...
  int LuaTurretSetRotation(lua_State* pt_lua_state) {
    ...
    /* Perform action */
    CLuaUtility::GetDeviceInstance<CCI_QupaTurretActuator>(pt_lua_state, "turret")->
  ...
  void CCI_QupaTurretActuator::SetActiveWithRotation(const CRadians& c_angle) {
    SetPositionControlMode();
    SetRotation(c_angle);
  }
  ...
  void CCI_QupaTurretActuator::SetSpeedControlMode() {
  ...
  void CCI_QupaTurretActuator::SetPositionControlMode() {
  ...
  void CCI_QupaTurretActuator::SetPassiveMode() {
...
#ifdef ARGOS_WITH_LUA
  void CCI_QupaTurretActuator::CreateLuaState(lua_State* pt_lua_state) {
