...
namespace argos {
   ...
   CDynamics2DQupaModel::CDynamics2DQupaModel(CDynamics2DEngine& c_engine,
      ...
      RegisterAnchorMethod<CDynamics2DQupaModel>(
         ...
         &CDynamics2DQupaModel::UpdateTurretAnchor);
      RegisterAnchorMethod<CDynamics2DQupaModel>(
         GetEmbodiedEntity().GetAnchor("perspective_camera"),
         &CDynamics2DQupaModel::UpdatePerspectiveCameraAnchor);
      ...
      m_ptBaseGripperAngularMotion->maxForce = QUPA_MAX_TORQUE; /* limit the dragging torque */
      /* Add the gripper body so that the default methods work as expected */
      AddBody(m_ptActualGripperBody, cpvzero, 0, QUPA_HEIGHT);
   ...
   void CDynamics2DQupaModel::MoveTo(const CVector3& c_position,
   ...
   void CDynamics2DQupaModel::UpdateFromEntityStatus() {
      ...
      if(m_cQupaEntity.GetTurretEntity().GetMode() != m_unLastTurretMode) {
         /* Enable or disable the anchor */
         if(m_cQupaEntity.GetTurretEntity().GetMode() != MODE_OFF) {
         ...
         switch(m_unLastTurretMode) {
            case MODE_OFF:
            case MODE_PASSIVE:
               switch(m_cQupaEntity.GetTurretEntity().GetMode()) {
                  case MODE_POSITION_CONTROL:
                  case MODE_SPEED_CONTROL:
      ...
      switch(m_unLastTurretMode) {
         ...
         case MODE_POSITION_CONTROL: {
            Real fCurRotErr = NormalizedDifference(
               m_cQupaEntity.GetTurretEntity().GetDesiredRotation(),
         ...
         case MODE_SPEED_CONTROL:
            m_ptControlGripperBody->w =
               m_cDiffSteering.GetAngularVelocity() +
               m_cQupaEntity.GetTurretEntity().GetDesiredRotationSpeed();
            break;
         case MODE_OFF:
         case MODE_PASSIVE:
   ...
   void CDynamics2DQupaModel::TurretPassiveToActive() {
      ...
      m_ptGripperControlAngularMotion->maxForce = QUPA_MAX_TORQUE; /* limit the dragging torque */
   }
