/**
 * @file <argos3/plugins/robots/qupa/simulator/qupa_light_rotzonly_sensor.cpp>
 *
...
#include "qupa_light_rotzonly_sensor.h"
...
namespace argos {
   ...
   CQupaLightRotZOnlySensor::CQupaLightRotZOnlySensor() :
   ...
   void CQupaLightRotZOnlySensor::SetRobot(CComposableEntity& c_entity) {
      try {
         ...
         THROW_ARGOSEXCEPTION_NESTED("Can't set robot for the qupa light default sensor", ex);
      }
   }
   ...
   void CQupaLightRotZOnlySensor::Init(TConfigurationNode& t_tree) {
      try {
         /* Show rays? */
   ...
   void CQupaLightRotZOnlySensor::Update() {
      ...
      /* Buffer for the angle of the light wrt to the qupa */
      CRadians cAngleLightWrtQupa;
       ...
       */
         for(auto it = mapLights.begin();
            ...
            if(cLight.GetIntensity() > 0.0f) {
               ...
               /* Check occlusion between the qupa and the light */
               if(! GetClosestEmbodiedEntityIntersectedByRay(sIntersection,
                  ...
                  /* Get the distance between the light and the qupa */
                  ...
                  /* Get the angle wrt to qupa rotation */
                  cAngleLightWrtQupa = cRobotToLight.GetZAngle();
                  cAngleLightWrtQupa -= cOrientationZ;
                  /*
                   * Find closest sensor index to point at which ray hits qupa body
                   * Rotate whole body by half a sensor spacing (corresponding to placement of first sensor)
                   * Division says how many sensor spacings there are between first sensor and point at which ray hits qupa body
                   * Increase magnitude of result of division to ensure correct rounding
                   */
                  Real fIdx = (cAngleLightWrtQupa - SENSOR_HALF_SPACING) / SENSOR_SPACING;
                  ...
                  for(SInt32 nIndexOffset = -6; nIndexOffset < 7; ++nIndexOffset) {
                     ...
                     CRadians cAngularDistanceFromOptimalLightReceptionPoint = Abs((cAngleLightWrtQupa - m_tReadings[unIdx].Angle).SignedNormalize());
                     /*
   ...
   void CQupaLightRotZOnlySensor::Reset() {
   ...
   REGISTER_SENSOR(CQupaLightRotZOnlySensor,
                   ...
                   "      <sensors>\n"
                   "        ...\n"
                   "        <qupa_light implementation=\"rot_z_only\" />\n"
                   "        ...\n"
                   "      </sensors>\n"
                   "      ...\n"
                   "    </my_controller>\n"
